...
func (seg *Segmenter) LoadDictionary(files string) error {
	...
	for _, file := range strings.Split(files, ",") {
		...
		ofiles = append(ofiles, dictFile)
		log.Printf("载入sego词典 %s", file)
		reader := bufio.NewReader(dictFile)
		readers = append(readers, reader)
	}
	err := seg.LoadDictionaryFromReaders(readers)
	if err != nil {
		return err
	}
...
func (seg *Segmenter) LoadDictionaryFromReaders(readers []io.Reader) error {
...
func splitTextToWords(text Text) []Text {
	...
	for current < len(text) {
		...
		if size <= 2 && (unicode.IsLetter(r) || unicode.IsNumber(r) || r == '_') {
			// 当前是拉丁字母或数字（非中日韩文字）
			if !inAlphanumeric {
